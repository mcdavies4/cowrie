...
export async function onboardStart(creator) {
  const s = stripe();
  let acct = creator.stripe_account_id;
...
// Per-transfer error handling: a single failure (or not-yet-settled balance in test
// mode / before funds clear) marks that split 'pending' for retry rather than throwing.
export async function distribute(deal, splits) {
  ...
  for (const split of splits) {
    ...
    try {
      ...
      // balance_insufficient happens when funds haven't settled to available yet — retryable.
      results.push({ split_id: split.id, transfer_id: null, status: 'pending', error: e.code || e.message });
    }
  }
  return results;
}
